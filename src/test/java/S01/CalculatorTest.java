...
// Здесь могут быть глобальные операции завершения
    ...
    @ParameterizedTest
    @ValueSource(ints = {0, 10, 47, 100})
    @DisplayName("Проверка процентов по скидке")
    void testDiscountByZero(int arg) {
        assertThrows(IllegalArgumentException.class, () -> calculator.calculateDiscount (arg, 0));
    }

    @ParameterizedTest
    @ValueSource(ints = {10, 47, 100})
    @DisplayName("Проверка процентов по скидке")
    void testDiscountBy(int Discount) {
        int arg = 5234;
        assertEquals(arg + (arg /100 * Discount), calculator.calculateDiscount (arg, Discount));
    }
